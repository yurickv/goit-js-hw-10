...
inputField.addEventListener("input", (0, _lodashDebounceDefault.default)(onSearch, DEBOUNCE_DELAY));
...
function onSearch(event) {
    ...
    getCountries(name).then((data)=>{
        createMark(data), console.log(data);
    }).catch((err)=>{
...
function createMark(arrey) {
    if (arrey.length > 10) {
        clearMark();
        return (0, _notiflixNotifyAio.Notify).info("Too many matches found. Please enter a more specific name.");
    }
    if (arrey.length > 1) {
        clearMark();
...
function createMarkList(countries) {
    return countries.map((element)=>`<li class="country-list-elem">
...
function createMarkInfo(country) {
    return country.map((element)=>`<div><img width="60" height="45" src="${element.flags.svg}" alt="${element.name.official}">
